    ...
    for (let e of tr.effects)
      if (e.is(cursorEffect)) {
        decorations = import_view.Decoration.set(
          e.value.map(
            (c) => import_view.Decoration.widget({
...
var VaultSyncManager = class {
  // Флаг, чтобы не зацикливать события
  constructor(plugin) {
    __publicField(this, "doc");
    __publicField(this, "filesMap");
    // Key: path, Value: { status: 'active' | 'deleted' }
    __publicField(this, "plugin");
    __publicField(this, "isProcessingRemote", false);
    this.plugin = plugin;
    this.doc = new Doc();
      ...
      for (const [path, meta] of Object.entries(remoteFiles)) {
        if (meta.status === "active") {
          if (!localPaths.has(path)) {
            ...
            if (folderPath && !this.plugin.app.vault.getAbstractFileByPath(
              folderPath
            )) {
            ...
            new import_obsidian.Notice(`[CyberSync] Downloaded: ${path}`);
          }
        } else if (meta.status === "deleted") {
          if (localPaths.has(path)) {
            const file = this.plugin.app.vault.getAbstractFileByPath(path);
            if (file) await this.plugin.app.vault.delete(file);
            new import_obsidian.Notice(`[CyberSync] Deleted remote: ${path}`);
          }
        }
      }
    } catch (e) {
  ...
  handleLocalCreate(file) {
    if (this.isProcessingRemote || !(file instanceof import_obsidian.TFile)) return;
    this.doc.transact(() => {
  ...
  handleLocalRename(file, oldPath) {
    if (this.isProcessingRemote || !(file instanceof import_obsidian.TFile)) return;
    this.doc.transact(() => {
...
var SyncPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    __publicField(this, "activeContentDoc", null);
    __publicField(this, "activeFilePath", null);
    // Менеджер структуры файлов
    __publicField(this, "vaultManager");
    // Очередь для синхронизации (5 сек)
    // Map<FilePath, Y.Doc>
    __publicField(this, "docsToSync", /* @__PURE__ */ new Map());
    __publicField(this, "syncIntervalId", null);
    __publicField(this, "clientColor", "#" + Math.floor(Math.random() * 16777215).toString(16));
  }
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on(
        "create",
        (f) => this.vaultManager.handleLocalCreate(f)
      )
    );
    this.registerEvent(
      this.app.vault.on(
        "delete",
        (f) => this.vaultManager.handleLocalDelete(f)
      )
    );
    this.registerEvent(
      this.app.vault.on(
        "rename",
        (f, old) => this.vaultManager.handleLocalRename(f, old)
      )
    );
    this.connect();
    this.registerEvent(
  ...
  async onLeafChange(leaf) {
    if (!leaf || !leaf.view || !(leaf.view instanceof import_obsidian.MarkdownView)) return;
  ...
  async initContentYjs(file, view) {
    this.activeFilePath = file.path;
    this.activeContentDoc = new Doc();
    ...
    const cursorSender = import_view.EditorView.updateListener.of((update) => {
  ...
  connect() {
    ...
    this.ws.onopen = () => {
      new import_obsidian.Notice("CyberSync Connected");
      this.sendWsMessage({
        type: "join_room",
    ...
    this.ws.onclose = () => {
      new import_obsidian.Notice("CyberSync Disconnected");
  ...
  async handleMessage(msg) {
    if (msg.type === "sync_update") {
      ...
      if (filePath === "__vault_index__") {
        applyUpdate(this.vaultManager.doc, update, "ws-sync");
        this.vaultManager.reconcileVault();
      } else if (filePath === this.activeFilePath && this.activeContentDoc) {
        applyUpdate(this.activeContentDoc, update, "ws-sync");
      }
    } else if (msg.type === "cursor" && msg.filePath === this.activeFilePath) {
      this.updateRemoteCursors(msg);
    }
  }
  // Добавляем документ в очередь на отправку
  queueSync(filePath, doc2) {
    this.docsToSync.set(filePath, doc2);
  }
  // Срабатывает каждые 5 секунд
  flushSyncQueue() {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) return;
    this.docsToSync.forEach((doc2, filePath) => {
